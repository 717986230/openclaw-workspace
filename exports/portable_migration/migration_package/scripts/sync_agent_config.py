...
REPO_PATH = BASE.as_posix()
QUOTED_REPO_PATH = f'"{REPO_PATH}"'
...
def transform_soul_text(text: str) -> str:
    text = text.replace('__REPO_DIR__', REPO_PATH)
    text = text.replace('/Users/bingsen/clawd/openclaw-sansheng-liubu/', f'{REPO_PATH}/')
    text = text.replace('/Users/bingsen/clawd/openclaw-sansheng-liubu', QUOTED_REPO_PATH)
    text = text.replace('python3 ', 'py -3 ')
    return text
...
def sync_scripts_to_workspaces():
    ...
    for proj_name, runtime_id in _SOUL_DEPLOY_MAP.items():
        workspace_root = pathlib.Path.home() / f'.openclaw/workspace-{runtime_id}'
        workspace_root.mkdir(parents=True, exist_ok=True)
        (workspace_root / '.edict_repo_root').write_text(str(BASE), encoding='utf-8')
        ws_scripts = workspace_root / 'scripts'
    ...
    ws_main_root = pathlib.Path.home() / '.openclaw/workspace-main'
    ws_main_root.mkdir(parents=True, exist_ok=True)
    (ws_main_root / '.edict_repo_root').write_text(str(BASE), encoding='utf-8')
    ws_main_scripts = ws_main_root / 'scripts'
...
def deploy_soul_files():
    ...
    for proj_name, runtime_id in _SOUL_DEPLOY_MAP.items():
        ...
        # 只在内容不同时更新（避免不必要的写入）
        src_text = transform_soul_text(src.read_text(encoding='utf-8', errors='ignore'))
        try:
        ...
        if src_text != dst_text:
            ...
            (ws_dst.parent / 'SOUL.md').write_text(src_text, encoding='utf-8')
            deployed += 1
