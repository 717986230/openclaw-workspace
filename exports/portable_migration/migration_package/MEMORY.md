...
<!-- openclaw-memory-promotion:memory:memory/2026-04-13.md:400:416 -->
- - Position documentation where users will find it - Simple changes can have big impact ### General Contribution Tips - Start with well-defined issues - Read existing code patterns carefully - Write comprehensive tests - Update documentation - Be responsive to feedback - Build relationships with maintainers --- **Date**: 2026-04-13 **Contributor**: Erbing **Status**: Active contribution to OpenClaw project (2 PRs submitted) [score=0.826 recalls=5 avg=0.538 source=memory/2026-04-13.md:400-416]
<!-- openclaw-memory-promotion:memory:memory/2026-04-11.md:439:498 -->
- - **Local Models**: Ollama integration for privacy --- ## 🤖 Agent Calling System ### Core Scripts - `scripts/agent_caller.py` - Main API for querying agents - `scripts/search-agent.py` - Search by keyword - `scripts/query-agents.py` - Query by category - `scripts/export_agents.py` - Export to JSON ### Database Integration - **Table**: `agent_prompts` in `memory/database/xiaozhi_memory.db` - **Total Agents**: 179 - **Categories**: 15 (Academic, Engineering, Marketing, etc.) - **Fields**: id, name, category, prompt, tags, created_at ### Usage Examples ```python # Search by keyword agents = search_agent("backend") # Query by category agents = query_agents("engineering") # Get random agent agent = get_random_agent() # Get full prompt prompt = get_agent_prompt("Backend Architect") ``` ### Demo Scenarios 1. **Backend Architect** - System design and architecture 2. **Code Reviewer** - Code quality and best practices 3. **Growth Hacker** - Marketing and user acquisition --- ## 🔧 Ollama Integration ### Models Supported - **nomic-embed-text** (768 dimensions) - Default - **mxbai-embed-large** (1024 dimensions) - High quality - **all-minilm** (384 dimensions) - Fast ### Implementation - `scripts/ollama_embedding.py` - Embedding provider - `skills/memory-system-complete/scripts/ollama_embedding.py` - Skill version - Integrated into `memory_system.py` with optional support ### Benefits - **Privacy**: Local processing, no external API calls - **Cost**: Free, no API fees - **Speed**: Fast inference on local GPU - **Reliability**: No rate limits or downtime --- [score=0.815 recalls=4 avg=0.624 source=memory/2026-04-11.md:439-498]
<!-- openclaw-memory-promotion:memory:memory/2026-04-13.md:29:32 -->
- openclaw cron add --id daily-brief --name "Daily Brief" --every 24h --message "Generate daily brief" openclaw cron edit daily-brief --no-deliver openclaw cron run daily-brief openclaw cron runs --id daily-brief [score=0.801 recalls=0 avg=0.620 source=memory/2026-04-13.md:29-32]
