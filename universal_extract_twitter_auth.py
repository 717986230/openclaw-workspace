...
class UniversalBrowserCookieExtractor:
    """通用浏览器Cookie提取器"""

    def __init__(self):
        """初始化提取器"""
        self.browser_paths = self._find_all_browser_paths()

    def _find_all_browser_paths(self) -> dict:
        """查找所有浏览器的Cookie文件路径"""
        paths = {
            'chrome': [],
            'edge': [],
                ...
                if os.path.exists(chrome_user_data):
                    for profile_dir in os.listdir(chrome_user_data):
                        if profile_dir.startswith('Profile ') or profile_dir == 'Default':
                            profile_cookies = os.path.join(chrome_user_data, profile_dir, 'Cookies')
                            if os.path.exists(profile_cookies):
                                paths['chrome'].append(('Chrome ' + profile_dir, profile_cookies))

                # Edge
                edge_user_data = os.path.join(local_app_data, 'Microsoft', 'Edge', 'User Data')
                if os.path.exists(edge_user_data):
                    for profile_dir in os.listdir(edge_user_data):
                        if profile_dir.startswith('Profile ') or profile_dir == 'Default':
                            profile_cookies = os.path.join(edge_user_data, profile_dir, 'Cookies')
                            if os.path.exists(profile_cookies):
                                paths['edge'].append(('Edge ' + profile_dir, profile_cookies))

                # Brave
                brave_user_data = os.path.join(local_app_data, 'BraveSoftware', 'Brave-Browser', 'User Data')
                if os.path.exists(brave_user_data):
                    for profile_dir in os.listdir(brave_user_data):
                        if profile_dir.startswith('Profile ') or profile_dir == 'Default':
                            profile_cookies = os.path.join(brave_user_data, profile_dir, 'Cookies')
                            if os.path.exists(profile_cookies):
                                paths['brave'].append(('Brave ' + profile_dir, profile_cookies))

            # Firefox (使用不同的Cookie存储格式)
            if app_data:
                ...
                if os.path.exists(firefox_profiles):
                    for profile_dir in os.listdir(firefox_profiles):
                        ...
                        if os.path.exists(cookies_file):
                            paths['firefox'].append(('Firefox ' + profile_dir, cookies_file))
        ...
        # macOS 路径
            ...
            if os.path.exists(chrome_path):
                paths['chrome'].append(('Chrome Default', chrome_path))

            # Edge
            edge_path = os.path.expanduser('~/Library/Application Support/Microsoft Edge/Default/Cookies')
            if os.path.exists(edge_path):
                paths['edge'].append(('Edge Default', edge_path))

            # Firefox
            firefox_path = os.path.expanduser('~/Library/Application Support/Firefox/Profiles/*/cookies.sqlite')
            firefox_files = glob.glob(firefox_path)
            for f in firefox_files:
                profile_name = os.path.basename(os.path.dirname(f))
                paths['firefox'].append(('Firefox ' + profile_name, f))
        ...
        # Linux 路径
            ...
            # Firefox
            firefox_path = os.path.expanduser('~/.mozilla/firefox/*/cookies.sqlite')
            firefox_files = glob.glob(firefox_path)
            for f in firefox_files:
                profile_name = os.path.basename(os.path.dirname(f))
                paths['firefox'].append(('Firefox ' + profile_name, f))
    ...
    def list_browser_paths(self):
        """列出所有找到的浏览器路径"""
        print("找到的浏览器Cookie文件:")
        print("=" * 60)

        found_any = False

        for browser, path_list in self.browser_paths.items():
            if path_list:
                found_any = True
                print(f"\n{browser.upper()}:")
                print("-" * 60)

                for i, (description, path) in enumerate(path_list, 1):
    ...
    def extract_from_path(self, cookies_path: str, browser_type: str = 'chrome') -> dict:
        """从指定路径提取Cookie"""
    ...
    def extract_twitter_cookies(self) -> dict:
        """提取Twitter Cookie（尝试所有浏览器）"""
        print("=" * 60)
        print("从浏览器提取Twitter认证信息")
        print("=" * 60)
        print()

        # 列出所有路径
        if not self.list_browser_paths():
            return {'success': False, 'error': '未找到任何浏览器的Cookie文件'}

        # 尝试从每个浏览器的每个路径提取
        for browser_type, path_list in self.browser_paths.items():
            if not path_list:
                continue

            print()
            print(f"尝试从 {browser_type.upper()} 提取...")
            print("=" * 60)

            for description, path in path_list:
                print()
                print(f"尝试: {description}")
                print("-" * 60)

                result = self.extract_from_path(path, browser_type)
...
def main():
    """主函数"""
    print()
    print("[Universal] Twitter 认证信息提取器")
    print("支持: Chrome, Edge, Firefox, Brave")
    print()

    # 创建提取器
    extractor = UniversalBrowserCookieExtractor()
    ...
    if cookies['success']:
        ...
        print(f"浏览器类型: {cookies.get('browser_type', 'Unknown')}")
