...
### 1. 数据导出
**文件**: `scripts/export_agents.py`

```python
# 从数据库导出179个Agent到JSON
agents = export_agents(db_path, 'data/agents.json')
```
...
```python
# 初始化时自动导入Agent
import_agents_from_json(db_path, 'data/agents.json')
```
...
```python
# 独立导入脚本
import_agents(db_path, json_path)
```
...
```bash
# 1. 克隆仓库
git clone https://github.com/717986230/agency-agents.git

# 2. 运行导入脚本
python scripts/import_agents.py --source ../agency-agents
