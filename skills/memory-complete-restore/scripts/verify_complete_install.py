...
def verify_installation(db_path: str = "memory/database/xiaozhi_memory.db") -> Dict:
    ...
    if not os.path.exists(db_path):
        ...
        print("  ✗ 数据库文件不存在")
        return result
    print(f"  ✓ 数据库文件存在 ({os.path.getsize(db_path) / 1024:.2f} KB)")
    ...
    try:
        ...
        if system.initialize():
            ...
            try:
                ...
                print(f"  ✓ 情感分析成功 ({emotion['primary_emotion']})")
            except Exception as e:
    ...
    try:
        ...
        if ollama.check_connection():
            ...
            print("  ⚠ Ollama服务不可用（可选）")
    except Exception as e:
        result['warnings'].append(f"Ollama检查失败: {e}（可选）")
        print(f"  ⚠ Ollama检查失败: {e}（可选）")
