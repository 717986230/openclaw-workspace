...
class AdvancedFuturesCrawler:
    ...
    def get_eastmoney_futures(self) -> List[Dict]:
        ...
        try:
            ...
            response = self.anti_detection.make_request(url, params=params)
        ...
        try:
            ...
            for contract in contracts:
                try:
                    ...
                    response = self.anti_detection.make_request(url)
    ...
    def save_to_database(self, data: List[Dict], source: str):
        ...
        try:
            task_id = self.db.create_task(
                title=f"{source} 高级爬取数据",
                url=f"advanced_{source.lower()}",
                description=f"使用高级反爬虫策略获取 {source} 期货数据"
            )
    ...
    def crawl_all(self) -> Dict[str, any]:
        ...
        # 显示反检测统计
        stats = self.anti_detection.get_stats()
        print()
        print("反检测统计:")
...
def main():
    ...
    try:
        ...
        # 保存到文件
        with open('advanced_futures_data.json', 'w', encoding='utf-8') as f:
            json.dump(results, f, ensure_ascii=False, indent=2)

        print()
        print("数据已保存到:")
        print("  - MySQL 数据库: crawler_db")
        print("  - JSON 文件: advanced_futures_data.json")
